...
//show the content
	...
	if ($show != 'qdetails' && $show != 'all') {
		echo button::create(['type'=>'button','label'=>$text['button-show_qdetails'],'icon'=>$settings->get('theme', 'button_icon_all'),'link'=>'?type=&show=qdetails'.(!empty($search) ? "&search=".urlencode($search) : null)]);	
	}
	...
	if (!empty($contacts)) {
		...
		if ($show == 'qdetails') {
			foreach($contacts as $contactRow) {
				...
				if ($contactRow['domain_uuid'] != $_SESSION['domain_uuid'] && permission_exists('domain_select')) {
					$list_row_url .= '&domain_uuid='.urlencode($contactRow['domain_uuid']).'&domain_change=true';
				}
